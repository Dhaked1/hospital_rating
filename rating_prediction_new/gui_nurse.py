...
st.write("Answer the following questions about nurse performance to predict the hospital's star rating.")

# Define answer options
answer_options = ["Always", "Usually", "Sometimes", "Never"]    

st.subheader("Nurse Communication")
nurse_communication = st.selectbox(
    "How often did nurses explain things clearly?",
    options=answer_options,
    key="nurse_communication"
)
st.subheader("Nurse Explanation")
nurse_explain = st.selectbox(
    "How often did nurse explain things in a way patients could understand?",
    options=answer_options,
    key="nurse_explain"
)
st.subheader("Nurse Listening")
nurse_listen = st.selectbox(
    "How often did doctors listen carefully to patients?",
    options=answer_options,
    key="nurse_listen"
)
st.subheader("Nurse Respect")
nurse_respect = st.selectbox(
    "How often did nurse treat patients with respect?",
    options=answer_options,
    key="nurse_respect"
)
...
if st.button("Predict Star Rating"):
    ...
    comm_percentages = map_survey_answer_to_percentages(nurse_communication)
    explain_percentages = map_survey_answer_to_percentages(nurse_explain)
    listen_percentages = map_survey_answer_to_percentages(nurse_listen)
    respect_percentages = map_survey_answer_to_percentages(nurse_respect)
